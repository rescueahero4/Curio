...
- **R-FE-9** — Library grid: 200 ms debounced search; infinite scroll via IntersectionObserver (400 px rootMargin) driving **keyset pagination** (`created_at|id` cursor, never offset); density toggle persisted in localStorage key `curio.dense`.
