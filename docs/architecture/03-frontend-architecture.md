...
- **R-FE-18** — The SPA MUST NOT serialize prompts. It stores TipTap doc JSON via PATCH; the serialized text (chip expansion, family exemplars, path embedding, newline collapsing, the write-back footer, sidecar snapshot) is produced solely by the server ([ARCH-01](01-backend-architecture.md); Inventory §6 "SERVER-side, authoritative"). Exemplars and the footer are document-scoped — a family offers its samples once per prompt and an item the user cited explicitly is not offered back — which is a further reason no client can re-derive the text chip by chip.

### Pairing fallback
- **R-FE-19** — `/pair` remains, as the **sanctioned fallback** pairing path (D11) for installs where the native-messaging bootstrap is unavailable ([ARCH-04](04-extension-architecture.md)). The DOM-handoff contract is unchanged from the old app: the secret MUST be absent from the DOM until the user clicks "Authorize this browser"; then it appears only in the hidden handoff element (`#curio-pairing-handoff`, `data-curio-pairing-token`); re-click is harmless (Inventory §6, §10.21). What the element hands off is the **per-run runtime token**, obtained by the click via the re-instated `POST /api/pair/authorize` ([ARCH-01](01-backend-architecture.md); token rules in [ARCH-06](06-security-architecture.md)).
- **R-FE-19a** — `/pair` MUST remain routed and MUST NOT be linked from the dashboard. Settings previously carried a "Browser extension" section linking to it with a full-document reload; it was removed. The state `/pair` resolves requires a **pinned port** (`CURIO_PORT` or `config.json`), which is not settable from the dashboard — so the link was a developer control on an end-user surface, and actively misleading in both states a user can reach: with native messaging registered the extension re-handshakes unaided, and on an ephemeral port the handoff fails anyway because `acceptPairingToken` has no port to bind the token to. The entry point and its four preconditions are documented in `web/extension/README.md`; anything reaching the page navigates to it directly, which preserves the full-document load that content-script injection depends on. Retiring `/pair` outright is blocked on the tray registering the native-messaging host on first launch (the P6 gap in `packaging/README.md`); until then it is the only channel that can deliver a token without NM.
