...
| Anthropic API key | Disk/log exposure | OS keychain only — no file fallback (D31); write-only API, masked display (R-SEC-10) |
...
- R-SEC-10: **Secrets.** The Anthropic API key is stored **keychain-only**: DPAPI (Windows; secret passed via env, never argv) or macOS Keychain. A platform with neither **refuses to store a key** and points at `ANTHROPIC_API_KEY`, which works everywhere. The AES-256-GCM encrypted-file fallback is **retired from v1** (D31, amended 2026-08-01): every release target in R-DEL-5 has a real keychain, so it was dead code on everything we ship, and its only purpose — reading the previous implementation's `.secrets.json` — required matching scrypt parameters that could not be verified without the artifact. A legacy `.secrets.json` is instead **detected and reported**, so a user upgrading re-enters a re-obtainable credential once rather than finding themselves silently keyless. The settings API is write-only for the key (`apiKey` accepted on PUT, never returned); reads expose only `apiKeySet` + masked `sk-ant-…xxxx`. The key MUST never enter the DB, sidecars, logs, or `runtime.json` (Inventory §5). Any public-settings projection MUST strip secrets structurally, not by schema omission alone — the Bun app's spread bug (Inventory §10.5) is the precedent.
...
- ~~OQ-3 (D0-verify; blocks: P1): keychain integration crates — verify the scrypt-fallback parameters match the previous implementation.~~ **Closed 2026-08-01 (D31).** Both backends are implemented and verified on Windows; the scrypt half is moot because the fallback it belonged to is retired. See R-SEC-10.
