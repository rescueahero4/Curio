...
| D36 | **Release asset filenames carry no version, and the footprint reports stop being release assets** (owner, 2026-08-03). Assets are `curio-windows-x64-setup.exe` and `curio-macos-universal.dmg`; the publish job names them literally rather than by glob | GitHub's only durable "newest release" URL is `/releases/latest/download/<asset-name>`, and it resolves by exact filename — a version in the name 404s every download button the moment the next tag ships, which is the one failure nobody notices until a user reports it. The version keeps five other carriers (`--version`, `/health`, `runtime.json` per R-DEL-12, plus `VIProductVersion` and `CFBundleShortVersionString`), so the filename was always the weakest of them and is the only one a stable URL forbids. No R-DEL rule constrains asset naming, so R-DEL-18 is not triggered. The footprint reports leave for a smaller reason: R-DEL-7 asks CI to produce and archive them, which the Actions artifact does, and three `.txt` diagnostics beside two installers dilute the one-click surface the landing page depends on | A release needs several versions downloadable at once, or a per-arch Windows build reintroduces a naming axis. For the reports: their 90-day artifact retention proves too short, at which point the ARCH-07 precedent is committing the numbers under `docs/architecture/` as D0-report.md does, not restoring them as assets |
| D35 | **The file-length ceiling rises 500 → 650, and stylesheets leave the check** (owner, 2026-08-02). R-DEL-6 step 7 amended; the warning tier moves to 500–650 | Two reasons of unequal quality, recorded separately because they reverse separately. The **raise is expedient**: four files crossed 500 during the variant-switcher work, they block every release artifact through the gate, and refactoring three of them under release pressure is how a decomposition gets done badly. 650 still catches a file nobody can hold in their head. The **stylesheet exemption is principled**: the rule's own rationale is that responsibilities accumulate where there are branches and review cost scales with paths through a file — a stylesheet has none, so counting it measured the wrong thing | The raise: the next time a file is split for real, put the ceiling back to 500 — it was never found to be the wrong number, only an inconvenient one. The exemption: a stylesheet language arrives that carries control flow |
