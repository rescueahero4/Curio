...
- `POST /api/system/open-skill-file`, `POST /api/system/quit` (auth `x-curio-quit-token` from lock file; 503/401/200), `POST /api/system/reveal {path}`.
...
- Settings page: pairing via `<Link to="/pair" reloadDocument>` (content script only injects on document load) + manual "Show pairing token"; key set/verify/clear; models; thresholds; MCP toggle + both snippets; autostart w/ unsupported reason; restartRequired notice.
- PromptEditor (TipTap → SolidJS equivalent needed): autosave; Copy Prompt = serialize→clipboard→markPromptSent (clipboard failure aborts before the claim is staked).
...
- Serialization is SERVER-side, authoritative: TipTap JSON → markdown-ish; chips: family → `Aesthetic: {name} — {description}` **Δ plus up to 3 exemplars `{name} ({id}) {abs dir}` and the visual-sense-not-a-template sentence, once per family per document, excluding items the document cites explicitly; degrades to the bare sentence when nothing is eligible**; tag/type → name; itemRef → `Reference: {name} — {abs dir} — read screenshot.png and item.md before designing; match feel, not content.`; collapse 3+ newlines; **Δ write-back footer appended to every prompt, naming `projectsRoot` and asking for one `project_register` call otherwise, surviving a fully emptied document**; snapshot `prompts/{id}.md` with "Edits here are not read back" header.
...
22. **GONE** — Send-to-Claude ordering (serialize→copy→claim→launch; "asked to open" phrasing). The button never transmitted the prompt: it copied to the clipboard and spawned `claude` with no arguments and stdin nulled. Removed rather than reimplemented; Copy Prompt keeps the surviving half, including the clipboard-failure abort.
